...
#define MAX_INSN_COUNT 5	// maximum instructions in a gadget

// print bytes in hex, 16 bytes per line
...
// print a found gadget's instructions with addresses
void print_gadget(cs_insn *insns, size_t count, size_t base_addr) {
	printf("Gadget @ 0x%lx:\n", base_addr);
	for (size_t i = 0; i < count; i++) {
		if (insns[i].mnemonic && insns[i].op_str)
			printf("  0x%lx:\t%s\t\t%s\n", insns[i].address, insns[i].mnemonic, insns[i].op_str);
		else
			printf("  0x%lx:\t<invalid instruction>\n", insns[i].address);
	}
	printf("\n");
}
...
int main(int argc, char **argv) {
	if (argc < 3) {
		...
		fprintf(stderr, "Supported arch: x86, x86_64, arm, arm64\n");
		return 1;
	}
	...
	// set Capstone architecture and mode based on input
	...
	// read section header string table (names)
	...
	// locate executable .text section
	...
	// initialize Capstone for given arch/mode
	...
	// scan section bytes for gadgets
	for (size_t offset = 0; offset < text_size; offset++) {
		...
		if (count > 0) {
			for (size_t len = 1; len <= count; len++) {
				...
				// check for return instructions per architecture
				if (arch == CS_ARCH_X86) {
					if (strcmp(last->mnemonic, "ret") == 0)
						is_rop_ret = 1;
				} else if (arch == CS_ARCH_ARM) {
					// common ARM returns: bx lr, pop {..., pc}, ret
					if ((strcmp(last->mnemonic, "bx") == 0 && strcmp(last->op_str, "lr") == 0) ||
						(strcmp(last->mnemonic, "ret") == 0) ||
						(strcmp(last->mnemonic, "pop") == 0 && strstr(last->op_str, "pc") != NULL))
						is_rop_ret = 1;
				...
				if (is_rop_ret) {
					...
					break;	// stop after first ret in this sequence
				}
			}
			cs_free(insn, count);
		}
